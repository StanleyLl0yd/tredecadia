...
The Russian profile maps canonical `YA /ja/` to `Я/я` and `ZU /zu/` to `ЗУ/зу`.

## Future profiles

Additional script profiles should be reviewed independently by competent speakers or readers before being marked stable. A future registry revision may add script/language aliases without changing canonical month identifiers.
