...
> **当前公开版本：`1.0.0-rc.2`。** 第二个候选版本引入了以国际中立性为目标设计的一周七天规范名称。稳定版 `v1.0.0` 只能在 RC2 单独完成观察期和最终复核之后发布。
...
这些名称是 Tredecadia 的规范标识符，并不是对星期一至星期日的翻译或改名。RC2 目前还没有经过审核的一周七天本地化别名，因此使用规范拉丁字母形式。
