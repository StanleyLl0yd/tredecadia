...
ROOT=Path(__file__).resolve().parents[1]; DATA=json.loads((ROOT/'registry/months.json').read_text()); MONTHS=DATA['months']; IPA=DATA['pronunciation']['syllableIpa']
...
def main():
 ...
 for m in MONTHS:
  ...
  once(l,f"| {i:02d} | {m['canonical']} | {m['localizations']['ru']} |",'localization')
 docs='\n'.join(t(p) for p in ['README.md','ROADMAP.md','specification/calendar-standard.md','specification/conversion-standard.md','specification/date-notation.md','specification/month-naming-standard.md','specification/localization.md','rationale/design.md','rationale/pronunciation.md'])
 ...
 for required in ['Tredecadia Era','year `0`','10000 BCE','Y - 9999','Y - 9998','astronomical','weak initial prominence','Short-4']: assert required in docs
