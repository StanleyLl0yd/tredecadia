...
> **目前公開版本：`1.0.0-rc.2`。** 第二個候選版本加入了以國際中立性為目標設計的標準星期日名稱。穩定版 `v1.0.0` 只能在 RC2 完成獨立觀察期與最後複核後發布。
...
這些名稱是 Tredecadia 的標準識別符，不是星期一到星期日的翻譯或重新命名。RC2 尚未定義經審核的本地化星期日別名，因此使用標準拉丁字母形式。
