...
Tredecadia 是一套開放的永久曆方案：一年分成 **13 個月，每月 28 天**。每個月剛好四週，同一個月日永遠落在同一個星期位置；用來補足太陽年的額外日則獨立於月份與七日星期週期之外。

> **目前公開版本：`1.0.0-rc.2`。** 第二個候選版本加入了以國際中立性為目標設計的一週七天標準名稱。穩定版 `v1.0.0` 只能在 RC2 完成獨立觀察期與最後複核後發布。

## 基本結構

- 13 × 28 = 364 個一般月內日。
- 每個月都是完整的四週。
- 每月 `01` 日固定是 **Mene (`W1`)**，`28` 日固定是 **Toze (`W7`)**。
...
這些名稱是 Tredecadia 的標準識別符，不是星期一到星期日的翻譯或重新命名。RC2 尚未定義經審核的一週七天本地化別名，因此使用標準拉丁字母形式。
